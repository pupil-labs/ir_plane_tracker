...
class AppWindow(QWidget):
    ...
    tracker_param_changed = Signal(str, object)
    ...
    def __init__(self, parent=None):
        ...
        self.raw_img_view = views.RawImageView(self.tracker_param_changed)
        self.tabs.addTab(self.raw_img_view, self.raw_img_view.name)

        self.thresholding_view = views.ThresholdingView(self.tracker_param_changed)
        self.tabs.addTab(self.thresholding_view, self.thresholding_view.name)

        self.contours_view = views.ContoursView(self.tracker_param_changed)
        self.tabs.addTab(self.contours_view, self.contours_view.name)

        self.fragment_view = views.FragmentsView(self.tracker_param_changed)
        self.tabs.addTab(self.fragment_view, self.fragment_view.name)

        self.ellipse_view = views.EllipseView(self.tracker_param_changed)
        self.tabs.addTab(self.ellipse_view, self.ellipse_view.name)

        self.feature_lines_view = views.FeatureLinesView(self.tracker_param_changed)
        self.tabs.addTab(self.feature_lines_view, self.feature_lines_view.name)

        self.optimization_view = views.OptimizationView(self.tracker_param_changed)
        self.tabs.addTab(self.optimization_view, self.optimization_view.name)

        # self.markers_view = MarkersView()
